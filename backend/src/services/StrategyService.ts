...
export class StrategyService extends BaseService {
  ...
  async createStrategy(data: {
    ...
    try {
      ...
      // Check if strategy name already exists for this alpha generator
      const existing = await StrategyModel.findByWallet(data.alphaGeneratorAddress || data.walletAddress)
      const nameExists = existing.some(s => s.strategyName === data.strategyName)

      if (nameExists) {
        throw new ApiError('Strategy name already exists for this alpha generator', 409)
      }

      // Prepare strategy data with all required fields
      const strategyData: NewStrategy = {
        ...
        alphaGeneratorAddress: data.alphaGeneratorAddress || data.walletAddress,
        isActive: true,
        subscriberCount: 0,
        totalVolume: "0"
      }
      ...
      this.log('Strategy created', { id: strategy.strategyId, wallet: data.walletAddress })
  ...
  async getStrategies(params: {
    ...
    isActive?: boolean
    walletAddress?: string
    protocol?: string  // Added protocol filter support
  }) {
    try {
      const { page = 1, limit = 20, isActive, walletAddress, protocol } = params
      ...
      if (walletAddress) {
        ...
        if (protocol) {
          strategies = strategies.filter(s =>
            s.supportedProtocols?.includes(protocol) ||
            s.strategyJSON?.protocol === protocol
          )
        }
  ...
  async getStrategyById(id: string) {
    try {
      ...
      const executionStats = await StrategyModel.getExecutionStats(strategy.strategyId)
  ...
  async deactivateStrategy(id: string, walletAddress: string) {
    try {
      ...
      if (strategy.alphaGeneratorAddress?.toLowerCase() !== walletAddress.toLowerCase()) {
